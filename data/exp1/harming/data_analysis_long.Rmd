...
# save(data1_cc, file = "~/Documents/Projects/harming_esn/data/data1_cc.Rdata")
```
...
## Model 5.1: Punishment prevalence threshold vs. decision time

```{r}
# Conflict: indicator for the threshold
m5.1 = lmer(behaviorTime_sec ~ conflict + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc)
tab_model(m5.1)
```

## Model 5.2: Punishment prevalence threshold and cooperation vs. decision time

```{r}
# Compare only cooperation vs punishment
data1_cc_no_defect = data1_cc %>% filter(behavior_defect == 0)
m5.2 = lmer(behaviorTime_sec ~ behavior_coop + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc_no_defect)
tab_model(m5.2)
```

## Model 5.3: Punishment prevalence threshold and defection vs. decision time

```{r}
# Compare only defection vs punishment
data1_cc_no_coop = data1_cc %>% filter(behavior_coop == 0)
m5.3 = lmer(behaviorTime_sec ~ behavior_defect + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc_no_coop)
tab_model(m5.3)
```

## Model 5.4: Punishment prevalence threshold vs. decision time, comparing punishers and non-punishers (both C+D)

```{r}
m5.4 = lmer(behaviorTime_sec ~ behavior_punish + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc)
tab_model(m5.4)
```

## Models 5.5, 5.6, 5.7, 5.8: Repeating models 5.1-5.4 using log decision time as the outcome

```{r}
# Model 5e
m5.5 = lmer(log_behaviorTime ~ showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + conflict + factor(round) + (1|game) + (1|superid), data = data1_cc)
tab_model(m5.5)
```

```{r}
# Model 5f
m5.6 = lmer(log_behaviorTime ~ behavior_coop + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc_no_defect)
tab_model(m5.6)
```

```{r}
# Model 5g
m5.7 = lmer(log_behaviorTime ~ behavior_defect + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc_no_coop)
tab_model(m5.7)
```

```{r}
## Model 5h
m5.8 = lmer(log_behaviorTime ~ behavior_punish + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc)
tab_model(m5.8)
```

## Models 5.6.1, 5.6.2, 5.7.1, 5.7.2, 5.8.1, 5.8.2: Restricting to only `conflict` == 1 or `conflict2` == 1

```{r}
# Model 5.6.1
m5.6.1 = lmer(log_behaviorTime ~ behavior_coop + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc_no_defect[conflict == 1,])
tab_model(m5.6.1)
```

```{r}
# Model 5.6.2
m5.6.2 = lmer(log_behaviorTime ~ behavior_coop + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc_no_defect[conflict2 == 1,])
tab_model(m5.6.2)
```

```{r}
# Model 5.7.1
m5.7.1 = lmer(log_behaviorTime ~ behavior_defect + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc_no_coop[conflict == 1,])
tab_model(m5.7.1)
```

```{r}
# Model 5.7.2
m5.7.2 = lmer(log_behaviorTime ~ behavior_defect + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc_no_coop[conflict2 == 1,])
tab_model(m5.7.2)
```

```{r}
## Model 5.8.1
m5.8.1 = lmer(log_behaviorTime ~ behavior_punish + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc[conflict == 1,])
tab_model(m5.8.1)
```

```{r}
## Model 5.8.2
m5.8.2 = lmer(log_behaviorTime ~ behavior_punish + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc[conflict2 == 1,])
tab_model(m5.8.2)
```

## Models 5.6.3, 5.7.3, 5.8.3: Repeating models 5.5, 5.6, 5.7 with `conflict` == 0

```{r}
# Model 5.6.3
m5.6.3 = lmer(log_behaviorTime ~ behavior_coop + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc_no_defect[conflict2 == 0,])
tab_model(m5.6.3)
```

```{r}
# Model 5.7.1
m5.7.1 = lmer(log_behaviorTime ~ behavior_defect + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc_no_coop[conflict2 == 0,])
tab_model(m5.7.1)
```

```{r}
## Model 5.8.2
m5.8.2 = lmer(log_behaviorTime ~ behavior_punish + showScore + age + gender + cPayoffS_lag + degree_lag + happ_lag + factor(round) + (1|game) + (1|superid), data = data1_cc[conflict2 == 0,])
tab_model(m5.8.2)
```
