...
save(ndata1,file="ndata1_0316_iX.Rdata") #This is used for the individual-level analysis. 
load("ndata1_0316_iX.Rdata") #ndata1 

dim(ndata1[ndata1$round==0,]) #745 individuals
dim(ndata1) #8772 individual-rounds (< 581*16 = 9296 individual-rounds for the complete cases) - how to get this number: now 10727?
...
network_data = ndata1[,c("gameID","game","round","superid","behavior_coop","behavior_defect", "behavior_punish","payoff","cumulativePayoff", "satisfaction","cPayoffS","behaviorTime","superid2.1","superid2.2","superid2.3","superid2.4","superid2.5","superid2.6","superid2.7","superid2.8","superid2.9","superid2.10","superid2.11","superid2.12","superid2.13","superid2.14")]

names(network_data)[3] = "previousround"
names(network_data)[5] = "e_behavior_coop"
names(network_data)[6] = "e_behavior_defect"
names(network_data)[7] = "e_behavior_punish"
names(network_data)[8] = "e_payoff"
names(network_data)[9] = "e_cumulativePayoff"
names(network_data)[10] = "e_satisfaction"
names(network_data)[11] = "e_stdPayoff"
names(network_data)[12] = "e_behaviorTime"


#Proc C: Preparing for the alter's attribute data
alter_data = ndata1[,c("gameID","game","round","superid","behavior_coop", "behavior_defect", "behavior_punish","payoff","cumulativePayoff" ,"satisfaction","cPayoffS")]
names(alter_data) = c("gameID","game","previousround","a_superid","a_behavior_coop", "a_behavior_defect", "a_behavior_punish","a_payoff","a_cumulativePayoff","a_satisfaction","a_cPayoffS")

#and for current round
alter_data2 = ndata1[,c("gameID","game","round","superid","behavior_coop", "behavior_defect", "behavior_punish","payoff","cumulativePayoff" ,"satisfaction","cPayoffS")]
names(alter_data2) = c("gameID","game","currentround","c_superid","c_behavior_coop", "c_behavior_defect", "c_behavior_punish","c_payoff","c_cumulativePayoff","c_satisfaction","c_cPayoffS")
...
hist(mdata1$cur_local_rate_coop)

hist(mdata1$prev_local_rate_defect)
hist(mdata1$cur_local_rate_defect)
...
save(mdata1,file="new_mdata1_0317X.Rdata")
save(mdata2,file="new_mdata2_03176X.Rdata")
...
write.csv(initial_data2, "initial_data2_0317X.csv",quote=FALSE,row.names=FALSE) 
mdata3 = merge(x=mdata2,y=initial_data2,all.x=TRUE,all.y=FALSE,by="superid")
write.csv(as.matrix(mdata3), "mdata3_0317X.csv",quote=FALSE,row.names=FALSE) #For STATA 
