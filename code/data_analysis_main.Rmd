...
length(unique(exp2data$superid))
length(unique(exp3data_pc$superid))
length(unique(exp4data$superid))
```
...
exp2data %>%
  filter(game %in% 1:20, time_pressure == "Minus", round > 0) %>%
  group_by(behavior)  %>%
  filter(behavior %in% c("C", "D", "P")) %>%
  count() %>%
  ungroup() %>%
...
exp2data %>%
  filter(game %in% 1:20, time_pressure == "Plus", round > 0) %>%
  group_by(behavior)  %>%
  filter(behavior %in% c("C", "D", "P")) %>%
  count() %>%
  ungroup() %>%
...
exp2data %>%
  filter(game %in% 21:50, time_pressure == "Plus", behavior %in% c("C", "D", "P")) %>%
  group_by(behavior) %>%
  count() %>%
  ungroup() %>%
  mutate(prop = n/sum(n))
...
exp2data %>%
  filter(game %in% 21:50, time_pressure == "Minus", behavior %in% c("C", "D", "P")) %>%
  group_by(behavior) %>%
  count() %>%
  ungroup() %>%
  mutate(prop = n/sum(n))
...
exp2data %>%
  filter(round >0, behavior %in% c("C", "D", "P"), game %in% 1:20) %>%
  group_by(behavior, time_pressure) %>%
  summarize(mean_dt = mean1(behaviorTime/1000))

exp2data %>%
  filter(round >0, behavior %in% c("C", "D", "P"), game %in% 21:50) %>%
  group_by(behavior, time_pressure) %>%
  summarize(mean_dt = mean1(behaviorTime/1000))
