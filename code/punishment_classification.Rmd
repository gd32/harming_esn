...
## Punishment for inequality aversion (IA)

- punishing to reduce inequality defined as punishment that occurs when ego's wealth is lower than the mean wealth of alters
...
## Reactive punishment (RP)

Punishment in response to punishment defined as punishment after an alter punished in last round (aka retaliating punishment)
...
## RP (-) - All punishment excluding RP

RP(-) defined as all punishment that is NOT retaliation (includes all IA and UP).

```{r}
tpdata_pc %>%
    filter(reactive_p == 0, behavior == "P") %>%
    group_by(tp_on) %>%
    summarize(n = n()) %>%
    mutate(armtotal = ifelse(tp_on == 0, 1580, 1629),
           armperc = n/armtotal)

#tp off: 173/1580 = 0.1095
#tp on: 137/1629 = 0.0841

prop.test(c(173, 137), c(1580, 1629)) # rough p-value = 0.0175
```
## Unexplained punishments - not reactive or calculated

```{r}
tpdata_pc %>%
  mutate(unexplained_p = ifelse(behavior == "P" & reactive_p == 0 & 
                             wealth_inequal == 0, 1, 0)) %>%
  group_by(tp_on, unexplained_p) %>%
  filter(unexplained_p %in% c(0, 1)) %>%
  summarize(n = n())
```
...
```{r}
# Making sure all punishment subtypes are in the dataset then saving to disk

# c("wealth_inequal", "reactive_p", "unexplained_p", "reactive_p_neg") %in% names(tpdata_pc)

tpdata_pc = tpdata_pc %>%
  mutate(reactive_p_neg = ifelse(reactive_p == 0 & behavior == "P", 1, 0),
