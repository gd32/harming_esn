...
## Together, but on the second scale
dt_data_by_behavior %>%
  mutate(mean_dt_punish_secs = mean_dt_punish * 1000,
         mean_dt_nonpunish_secs = mean_dt_nonpunish * 1000) %>%
  ggplot() + 
  geom_density(aes(x = mean_dt_punish_secs, y = ..scaled..), color = "orange") +
  geom_density(aes(x = mean_dt_nonpunish_secs, y = ..scaled..), color = "blue")+
  xlab("Seconds")+
  ylab("Density") 
...
dt_data_by_behavior %>%
  mutate(mean_dt_punish_secs = mean_dt_punish/1000,
         mean_dt_nonpunish_secs = mean_dt_nonpunish/1000,
         mean_punish_secs_log10 = log10(mean_dt_punish_secs),
         mean_nonpunish_secs_log10 = log10(mean_dt_nonpunish_secs)) %>%
  summarize(mean_punish = mean1(mean_dt_punish_secs),
            mean_nonpunish = mean1(mean_dt_nonpunish_secs),
            median_punish = median(mean_dt_punish_secs, na.rm = T),
            median_nonpunish = median(mean_dt_nonpunish_secs, na.rm = T))
...
dt_data_by_behavior %>%
  mutate(mean_dt_punish_secs = mean_dt_punish/1000,
         ...
         mean_nonpunish_secs_log10 = log10(mean_dt_nonpunish_secs)) %>%
  ggplot() + 
  geom_density(aes(x = mean_punish_secs_log10, y = ..scaled..), color = "orange") +
  geom_density(aes(x = mean_nonpunish_secs_log10, y = ..scaled..), color = "blue")+
  xlab("Seconds (log10)")+
  ylab("Density") +
  xlim(0, 2.5)
```
...
## Individual-level regression using individual peace status as the outcome (i.e. no local-network punishment, ego is not a punisher)
