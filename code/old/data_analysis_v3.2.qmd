...
Here we create the single-variable behavior and lagged behavior variables.
...
exp4data %>%
  filter(time_pressure == "Minus", round >=1) %>% 
  group_by(game) %>%
  select(superid) %>%
  unique() %>%
  summarize(n = n()) %>%
...
```{r}
# Behavior breakdown - Experiment 1
...
```{r}
# Behavior breakdown - Experiment 2
exp4data %>%
  group_by(behavior) %>%
...
### Exp 2, TP-: Regression model with and without punishment
...
### Exp 2, TP+: Regression model with and without punish in last round
