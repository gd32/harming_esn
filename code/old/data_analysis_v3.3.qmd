...
```{r}
## Experiment 1
load("~/Documents/Projects/harming_esn/data/final/exp1data.Rdata")

## Experiment 2
load("~/Documents/Projects/harming_esn/data/final/exp2data.Rdata")
```
...
exp2data %>%
  ...
  ungroup() %>%
  mutate(proportion = count/sum(count)) %>%
  rename(`Behavior` = behavior, `Count` = count, `Proportion` = proportion)

exp1data_CI = MultinomCI(c(4185, 5790, 679), sides = "two.sided")
```
...
```{r}
m2 = glmer(behavior_punish ~ time_pressure + round + (1|game) + (1|superid),  data = exp2data %>% filter(round > 0), family = binomial, nAGQ=0, control = glmerControl(optimizer = c("bobyqa"), optCtrl=list(maxfun=2e5), calc.derivs=FALSE))
...
```{r}
m2.1 = glmer(behavior_coop ~ time_pressure + round + (1|game) + (1|superid),  data = exp2data %>% filter(round > 0), family = binomial, nAGQ=0, control = glmerControl(optimizer = c("bobyqa"), optCtrl=list(maxfun=2e5), calc.derivs=FALSE))
summary(m2.1)
```
...
```{r}
m2.2 = glmer(behavior_defect ~ time_pressure + round + (1|game) + (1|superid),  data = exp2data %>% filter(round > 0), family = binomial, nAGQ=0, control = glmerControl(optimizer = c("bobyqa"), optCtrl=list(maxfun=2e5), calc.derivs=FALSE))
...
```{r}
model_dt_coop = lmer(behaviorTime_sec ~ time_pressure + round + (1|game) + (1|superid),  data = exp2data %>% filter(round > 0, behavior_coop == 1))
summary(model_dt_coop)
```

```{r}
model_dt_def = lmer(behaviorTime_sec ~ time_pressure + round + (1|game) + (1|superid),  data = exp2data %>% filter(round > 0, behavior_defect == 1))
summary(model_dt_def)
```

```{r}
model_dt_pun = lmer(behaviorTime_sec ~ time_pressure + round + (1|game) + (1|superid),  data = exp2data %>% filter(round > 0, behavior_punish == 1))
...
```{r}
m3.1 = glmer(punish_type_CR ~ time_pressure + round + (1|game) + (1|superid),  data = exp2data %>% filter(round > 0), family = binomial, nAGQ=0, control = glmerControl(optimizer = c("bobyqa"), optCtrl=list(maxfun=2e5), calc.derivs=FALSE))
...
```{r}
m3.2 = glmer(punish_type_NR ~ time_pressure + round + (1|game) + (1|superid),  data = exp2data %>% filter(round > 0), family = binomial, nAGQ=0, control = glmerControl(optimizer = c("bobyqa"), optCtrl=list(maxfun=2e5), calc.derivs=FALSE))
...
```{r}
m3.3 = glmer(punish_type_IA ~ time_pressure + round + (1|game) + (1|superid),  data = exp2data %>% filter(round > 0), family = binomial, nAGQ=0, control = glmerControl(optimizer = c("bobyqa"), optCtrl=list(maxfun=2e5), calc.derivs=FALSE))
...
```{r}
m3.4 = glmer(punish_type_U ~ time_pressure + round + (1|game) + (1|superid),  data = exp2data %>% filter(round > 0), family = binomial, nAGQ=0, control = glmerControl(optimizer = c("bobyqa"), optCtrl=list(maxfun=2e5), calc.derivs=FALSE))
...
m5a = lmer(behaviorTime_sec ~ behavior + round + (1|game) + (1|superid), data = exp2_tp_plus %>% filter(round > 0))
summary(m5a)
```

```{r}
m5b = lmer(behaviorTime_sec ~ behavior + round + (1|game) + (1|superid), data = exp2_tp_minus %>% filter(round > 0))
...
exp2_tp_plus_fig3_data = bind_cols(exp2data_tp_plus_count, exp2data_tp_plus_CI, exp2data_tp_plus_times)[-7]
...
exp2_tp_minus_fig3_data = bind_cols(exp2data_tp_minus_count, exp2data_tp_minus_CI, exp2data_tp_minus_times)[-7]
...
exp2data_times = bind_rows(exp2data_minus_CR_times, exp2data_minus_IA_times, exp2data_minus_NR_times, exp2data_minus_U_times, exp2data_plus_CR_times, exp2data_plus_IA_times, exp2data_plus_NR_times, exp2data_plus_U_times)
```

```{r}
exp2_times_old = exp2data_times %>%
