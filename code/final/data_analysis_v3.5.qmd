...
We are interested here in the power of Experiment 2 to detect differences in the rate of cooperation. We therefore use

```{r}
exp2data %>% group_by(time_pressure) %>% filter(round > 0) %>% count()
...
exp(fixef(m1.1)['time_pressurePlus'] )

library(simr)
fixef(m1.1)['time_pressurePlus'] = 0.0411

powerSim(m1.1, nsim = 10)

pwr.2p2n.test(h = ES.h(p1 = 0.494, p2 = 0.494*1.117), n1 = 5292, n2 = 5455, sig.level = 0.05, power = NULL, alternative = 'two.sided')
```
...
```{r}
m4 = lmer(behaviorTime_sec ~ behavior + round + (1|game) + (1|superid), data = exp1data)
summary(m4)
```
