...
#### Exp 1 - Testing interaction between punishing environment and decision-making

```{r}
quantile(exp1data$local_rate_punish_lag, c(0.33, 0.66, 0.99), na.rm = T)

median(exp1data$local_rate_punish_lag, na.rm = T)
...
```
```{r}
m4_no_int_tp_minus = lmer(behaviorTime_sec ~ behavior_punish + local_rate_punish_lag + 
                round + (1|game) + (1|superid), 
              data = exp2data %>% filter(round > 1, time_pressure == 'Minus'))

m4_int_tp_minus = lmer(behaviorTime_sec ~ behavior_punish*local_rate_punish_lag + 
                round + (1|game) + (1|superid), 
              data = exp2data %>% filter(round > 1, time_pressure == 'Minus'))
summary(m4_no_int_tp_minus)
summary(m4_int_tp_minus)
anova(m4_no_int_tp_minus, m4_int_tp_minus)

m4_no_int_tp_plus = lmer(behaviorTime_sec ~ behavior_punish + local_rate_punish_lag + 
                round + (1|game) + (1|superid), 
              data = exp2data %>% filter(round > 1, time_pressure == 'Plus'))

m4_int_tp_plus = lmer(behaviorTime_sec ~ behavior_punish*local_rate_punish_lag + 
                round + (1|game) + (1|superid), 
              data = exp2data %>% filter(round > 1, time_pressure == 'Plus'))

anova(m4_no_int_tp_plus, m4_int_tp_plus)
```
...
##### Evaluating the effect of wealth visibility on frequency of behaviors in Experiment 1

```{r}
load("../exp1/data1_pc_v2.Rdata")

player_id_by_initial_score = data1_pc %>% select(superid, initial_score) %>% distinct() %>%
  mutate(initial_score_low = ifelse(initial_score == 200, 1, 0)) %>%
  select(superid, initial_score_low)

exp1data = exp1data %>% left_join(player_id_by_initial_score, by = 'superid')
```

##### Stratify the data by initial score

```{r}
# Counts, CIs, and decision times for low initial allocation 
data1_behavior_count_low_score = exp1data %>%
  filter(round >= 1, initial_score_low == 1) %>%
  group_by(behavior) %>%
  filter(behavior %in% c("C", "D", "P")) %>%
  summarize(count = n()) %>%
  ungroup() %>%
...
data1_behavior_CI_low_score = MultinomCI(x = c(3501, 2939, 378), sides = "two.sided") %>%
  as_tibble()

data1_times_low_score = exp1data %>%
  group_by(behavior) %>%
  filter(behavior %in% c("C", "D", "P"), initial_score_low == 1) %>%
  summarize(mean_dt = mean1(behaviorTime_sec),
            se_mean_dt = se_mean(behaviorTime_sec),
            LL_mean = mean_dt - 1.96*se_mean_dt,
            UL_mean = mean_dt + 1.96*se_mean_dt)

exp1_low_score_data = bind_cols(data1_behavior_count_low_score, data1_behavior_CI_low_score, data1_times_low_score)[-7] 
names(exp1_low_score_data) = c("behavior", "count", "crude_prop", "adjusted_prop", 
                          "LL_prop", "UL_prop", "mean_dt", "se_mean_dt", 
                          "UL_mean_dt", "LL_mean_dt")

# Counts, CIs, and decision times for high initial allocation 
data1_behavior_count_high_score = exp1data %>%
  filter(round >= 1, initial_score_low == 0) %>%
  group_by(behavior) %>%
  filter(behavior %in% c("C", "D", "P")) %>%
  summarize(count = n()) %>%
  ungroup() %>%
...
data1_behavior_CI_high_score = MultinomCI(x = c(1377, 1397, 184), sides = "two.sided") %>%
  as_tibble()

data1_times_high_score = exp1data %>%
  group_by(behavior) %>%
  filter(behavior %in% c("C", "D", "P"), initial_score_low == 0) %>%
  summarize(mean_dt = mean1(behaviorTime_sec),
            se_mean_dt = se_mean(behaviorTime_sec),
            LL_mean = mean_dt - 1.96*se_mean_dt,
            UL_mean = mean_dt + 1.96*se_mean_dt)

exp1_high_score_data = bind_cols(data1_behavior_count_high_score, data1_behavior_CI_high_score, data1_times_high_score)[-7] 
names(exp1_high_score_data) =  c("behavior", "count", "crude_prop", "adjusted_prop", 
                          "LL_prop", "UL_prop", "mean_dt", "se_mean_dt", 
                          "UL_mean_dt", "LL_mean_dt")
```
...
-   No association between point allocation and decision time in general

```{r}
summary(lmer(behaviorTime_sec ~ initial_score_low + round + (1|game) + (1|superid), data = exp1data))
```

-   Additionally, no association between point allocation and decision time accounting for behavior type

```{r}
summary(lmer(behaviorTime_sec ~ initial_score_low + behavior + round + (1|game) + (1|superid), data = exp1data))
```

##### Evaluating the effect of wealth visibility on the decision times of behaviors

```{r}
load('../../data/exp4/exp4data.Rdata')
player_id_by_initial_score_e2 = exp4data %>% select(superid, initial_score) %>% distinct() %>%
  mutate(initial_score_low = ifelse(initial_score == 200, 1, 0)) %>%
  select(superid, initial_score_low)

exp2data = exp2data %>% left_join(player_id_by_initial_score_e2, by = 'superid')
```

```{r}
summary(lmer(behaviorTime_sec ~ initial_score_low + behavior + round + (1|game) + (1|superid), data = exp2data %>% filter(time_pressure == 'Minus')))
```

```{r}
summary(lmer(behaviorTime_sec ~ behavior + initial_score_low + round + (1|game) + (1|superid), data = exp2data %>% filter(time_pressure == 'Plus')))
```

```{r}
summary(lmer(behaviorTime_sec ~ punish_type_NR + punish_type_IA + punish_type_U + initial_score_low + 
              round + (1|game) + (1|superid), 
            data = exp1data %>% filter(behavior_punish == 1)))
```
...
summary(lmer(behaviorTime_sec ~ punish_type_NR + punish_type_IA + punish_type_U + initial_score_low + 
              round + (1|game) + (1|superid), 
            data = exp2data %>% filter(behavior_punish == 1, time_pressure == 'Minus')))
...
summary(lmer(behaviorTime_sec ~ punish_type_NR + punish_type_IA + punish_type_U + initial_score_low + 
              round + (1|game) + (1|superid), 
            data = exp2data %>% filter(behavior_punish == 1, time_pressure == 'Plus')))
```

##### Stratified Analysis of Inequality Aversion Punishment

```{r}
exp1data %>% filter(punish_type_IA == T) %>% mutate(high_defect_rate_lag = ifelse(local_rate_defect_lag > 0.5, 'Defect rate > 0.5', "Defect rate <= 0.5")) %>% group_by(high_defect_rate_lag) %>%
  summarize(n = n(),
            mean_dt = mean1(behaviorTime_sec))

IA_data_e1 = exp1data %>% filter(punish_type_IA == T) %>% mutate(high_defect_rate_lag = ifelse(local_rate_defect_lag > 0.5, 'Defect rate > 0.5', "Defect rate <= 0.5")) 

summary(lmer(behaviorTime_sec ~ high_defect_rate_lag + round + (1|game) + (1|superid), data = IA_data_e1))
```

```{r}
IA_data_e2 = exp2data %>% filter(punish_type_IA == T) %>% mutate(high_defect_rate_lag = ifelse(local_rate_defect_lag > 0.5, 'Defect rate > 0.5', "Defect rate <= 0.5")) 

summary(lmer(behaviorTime_sec ~ high_defect_rate_lag + round + (1|game) + (1|superid), data = IA_data_e2 %>% filter(time_pressure == 'Minus')))
```

```{r}
summary(lmer(behaviorTime_sec ~ high_defect_rate_lag + round + (1|game) + (1|superid), data = IA_data_e2 %>% filter(time_pressure == 'Plus')))
```

##### Testing the reciprocal effect of cooperation and defection decisions

```{r}
# Reciprocation occurs when previous round local rate is >0
# Check cooperation, Exp. 1/Exp.2 TP-/Exp.2 TP+

exp1data = exp1data %>% mutate(any_defectors_lag = ifelse(local_rate_defect_lag > 0, 1, 0))
exp2data = exp2data %>% mutate(any_defectors_lag = ifelse(local_rate_defect_lag > 0, 1, 0))
```

```{r}
summary(lmer(behaviorTime_sec ~ any_defectors_lag + round + (1|game) + (1|superid), data = exp1data %>% filter(behavior_defect == 1, round > 0)))
```

```{r}
summary(lmer(behaviorTime_sec ~ any_defectors_lag + round + (1|game) + (1|superid), data = exp2data %>% filter(behavior_defect == 1, round > 0, time_pressure == 'Minus')))
```

```{r}
summary(lmer(behaviorTime_sec ~ any_defectors_lag + round + (1|game) + (1|superid), data = exp2data %>% filter(behavior_defect == 1, round > 0, time_pressure == 'Plus')))
```


##### Compare only punishment decisions where no overlap occurred

```{r}
nr_no_overlap_times = exp1data %>% filter(punish_type_NR == 1, punish_type_IA == 0, punish_type_CR == 0, punish_type_U == 0) %>% 
    summarize(mean_dt = mean1(behaviorTime_sec),
            se_mean_dt = se_mean(behaviorTime_sec),
            n = n()) %>%
...
ia_no_overlap_times = exp1data %>% filter(punish_type_NR == 0, punish_type_IA == 1, punish_type_CR == 0, punish_type_U == 0) %>% 
    summarize(mean_dt = mean1(behaviorTime_sec),
            se_mean_dt = se_mean(behaviorTime_sec),
             n = n()) %>%
...
cr_no_overlap_times = exp1data %>% filter(punish_type_NR == 0, punish_type_IA == 0, punish_type_CR == 1, punish_type_U == 0) %>% 
    summarize(mean_dt = mean1(behaviorTime_sec),
            se_mean_dt = se_mean(behaviorTime_sec),
             n = n()) %>%
...
u_no_overlap_times = exp1data %>% filter(punish_type_NR == 0, punish_type_IA == 0, punish_type_CR == 0, punish_type_U == 1) %>% 
    summarize(
          mean_dt = mean1(behaviorTime_sec),
            se_mean_dt = se_mean(behaviorTime_sec),
           n = n()) %>%
...
bind_rows(nr_no_overlap_times, ia_no_overlap_times, cr_no_overlap_times, u_no_overlap_times)

```

## Evaluating effect of wealth visibility on IA punishment

```{r}
gameid_by_wealth_vis = data1_pc %>% select(game, showScore) %>% distinct() 

exp1data = exp1data %>% left_join(gameid_by_wealth_vis, by = 'game')

exp1data %>% filter(showScore == 0) %>% 
...
```{r}
exp1data %>% 
  filter(punish_type_IA == 1, showScore == 0, behavior_punish == 1) %>%
  summarize(mean_dt = mean1(behaviorTime_sec),
            se_mean_dt = se_mean(behaviorTime_sec),
) %>%
...
```{r}
exp1data %>% 
  filter(punish_type_IA == 1, showScore == 1, behavior_punish == 1) %>%
  summarize(mean_dt = mean1(behaviorTime_sec),
            se_mean_dt = se_mean(behaviorTime_sec),
) %>%
  mutate(punish_type = "IA",   mean_LL = mean_dt - 1.96*se_mean_dt,
...
```{r}
summary(glmer(punish_type_IA ~ showScore + round + (1|game) + (1|superid), data = exp1data %>% filter(round > 0), family = 'binomial', nAGQ=0, 
      control = glmerControl(optimizer = c("bobyqa"), 
                                  optCtrl=list(maxfun=2e5), 
                                  calc.derivs=FALSE)))
```
...
```{r}
summary(lmer(behaviorTime_sec ~ showScore + round + (1|game) + (1|superid), data = exp1data %>% filter(punish_type_IA == 1)))
```
