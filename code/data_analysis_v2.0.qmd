...
## Figure 1 - Behavior Frequencies and Decision Time, Experiment A
...
exp1_fig1_B = exp1_fig1_data %>%
  ...
  geom_bar(stat = "identity", width = 0.75, alpha = 0.5) +
  geom_errorbar(aes(ymin = recentered_mean_dt - se_mean_dt, 
                    ymax = recentered_mean_dt + se_mean_dt, 
...
## Figure 1 - Behavior frequencies and Decision Time, Experiment B
...
names(exp4_tp_plus_fig1_data) = c("behavior", "count", "crude_prop", 
                                  ...
                                  "se_mean_dt")
exp4_tp_plus_fig1_data = exp4_tp_plus_fig1_data %>%
  mutate(behavior = ifelse(behavior == "P", "H", behavior))
...
exp4_tp_minus_fig1_data = exp4_tp_minus_fig1_data %>%
  mutate(behavior = ifelse(behavior == "P", "H", behavior))
```
...
```{r}
(exp1_fig1_A / exp1_fig1_B) +
  plot_annotation(title = "Behavior frequency and decision times, Experiment A",
                  theme = theme(plot.title = element_text(size = 12)))

ggsave(filename = "~/Documents/Projects/harming_esn/figures/fig1_top_v2.2.png", width = 8, height = 6, units = "in")

(exp4_fig1_A + exp4_fig1_C) / (exp4_fig1_B + exp4_fig1_D) +
  plot_annotation(title = "Behavior frequency and decision times, Experiment B, TP+ (left) and TP- (right)", theme = theme(plot.title = element_text(size = 12)))

ggsave(filename = "~/Documents/Projects/harming_esn/figures/fig1_bottom_v2.2.png", width = 8, height = 6, units = "in")
```
...
m6 = lmer(cumulativePayoff ~ time_pressure + factor(round) + (1|game) + (1|superid), data = exp4_modeling)
