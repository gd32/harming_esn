rm(list = ls())
setwd("~/Documents/Projects/harming_esn/question_time_v1.1/harming/")
load(file="node_harming.Rdata") #ndata
load(file="link_harming.Rdata") #ldata
...
ndata_times$game = 200 + ndata_times$game
...
harm_times = ndata_times %>%
  select(superid, ends_with("time")) %>%
  rename(start_time = finalscore_time)
harm_times

write_csv(harm_times, file = "~/Library/CloudStorage/Dropbox/2023S/PANAS/time_data/harm_times.csv")

happ_data$game = 00 + happ_data$game
