rm(list = ls())
setwd("~/Documents/Projects/harming_esn/Question-time/happiness")
...
ndata_times = ndata1 %>% as_tibble() %>%
  filter(round == 15) %>%
  select(game, id, q1_time, q2_time, q3_time, q4_time, gender_time, age_time) %>%
  unnest(cols = c(q1_time, q2_time, q3_time, q4_time, gender_time, age_time))
